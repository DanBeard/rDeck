...
// Maybe one day, but just manual for now

// #define BYTE_PRECISION 24
...
// static int32_t next_value_sd(File &file) {
...
// static int32_t next_value_sd(File &file, uint32_t &position) {
//   //Serial.print(__func__);
//   uint8_t marker;
//   int32_t value;
...
// static bool check_inside_shape_sd(File &file, uint32_t shape_position, int32_t latitude_int, int32_t longitude_int) {
//   Serial.print(__func__);
...
//     //Serial.printf("k P2: %d %f %f\n", k, int_to_float(p2.latitude, 90, BYTE_PRECISION), int_to_float(p2.longitude, 180, BYTE_PRECISION));
...
//             //Serial.printf("Crossed line P1: %f %f   P2: %f %f\n", int_to_float(p1.latitude, 90, BYTE_PRECISION), int_to_float(p1.longitude, 180, BYTE_PRECISION), int_to_float(p2.latitude, 90, BYTE_PRECISION), int_to_float(p2.longitude, 180, BYTE_PRECISION));
//           }
//         }
//       }
//     }
...
// static bool check_inside_country_sd(File &file, uint32_t data_position, int32_t latitude_int, int32_t longitude_int) {
...
//   File file = SD.open("/timezones.bin", FILE_READ);
...
//   return &sd->parent;
// }
// void TimeHelper::setLocation(double lat, double lon) {
//   Serial.println("Grabbing timzeone info....");
//   static tz_database_conf_t tz_database_conf = {};

//   tz_database_s *tzdb = tz_database_new_sd(&tz_database_conf);
//   tzdb->init(tzdb);

//   char *timezone = tzdb->find_timezone(tzdb, 41.87957f,-87.62381f);
